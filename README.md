**English** | [中文](README-cn.md) | [PDF (EN)](GPUs-Wombs-and-Money-Printers-v1.0.2.pdf) | [PDF (中文)](GPU、子宫与印钞机-v1.0.2.pdf)

# GPUs, Wombs, and Money Printers: The Fates of Human Societies in the Age of AI

---

## I. Introduction: From Cat-vs-Dog to Passing the Bar—A Decade Apart

In 2012 AlexNet won the ImageNet contest by “telling cats from dogs” [3]. Most people saw it as an academic breakthrough; few expected an industry-scale revolution to arrive this fast. Ten years later, on Nov 30, 2022, ChatGPT went live—1 million users in five days, 100 million in two months. It writes poems, code, and passes the U.S. bar and medical licensing exams. In one decade, AI leapt from classifying pets to acing the hardest human exams; our adaptation window is likely much shorter than ten years.

Each tech wave is faster: AT&T dominated telephony ~50 years, IBM mainframes ~30, Microsoft PCs ~20, Google the web ~15 [2]. This time the wave isn’t giving humans mechanical arms—it’s giving machines a human brain. The target has moved from hands and feet to cognition, and the clock is ticking.

**Promise of this essay:** not sci‑fi prediction, but data‑anchored simulation: if today’s trends keep running, what happens in 5, 15, even 100 years across economy, geopolitics, demography, finance, and the species itself? You’ll leave with a set of leading indicators, a timeline, and an action checklist.

![Tech wave compression](charts/en/wave-timeline.svg)

This chart’s takeaway: each dominance cycle shrinks—your decision window may be under ten years.

Speed is the surface; the deeper story is which **human capability layer** gets eaten. See below.

![From muscles to minds](charts/en/automation-layers.svg)

The layers: steam engines (coarse muscle) → electrification/assembly (rhythm) → industrial robots (fine repetitive motion) → PCs/Internet (calculation & memory) → deep learning (perception) → LLMs/agents (general cognition) → embodied AI (cognition + action). Each wave is faster and targets a higher layer.

In three short years, LLMs have gone from “chatty search” to autonomous coding/research/decision agents. Citrini Research’s Feb 2026 report “The Consequences of Surplus Intelligence” asks: standing in June 2028, what will we see? [1]

This essay pushes further—a 100‑year simulation across economic structure, geopolitics, population, finance, and the human species. It’s not a forecast; it’s a chain of “if trends continue” logic.

---

## II. The Collapse of Intelligence Premium: When Human Brains Aren’t Scarce

### 2.1 The Human-Intelligence Displacement Spiral

For 200 years, economies assumed cognition is scarce. Lawyers earn six figures because few can pass the bar. Same for programmers, analysts, consultants. AI is ending that scarcity.

By late 2025, agentic coding tools jumped a step. A mid-level dev with Claude Code or Codex can rebuild a mid-size SaaS core in weeks. CIOs ask: “Why buy? Let’s build.” Long-tail SaaS (Monday, Zapier, Asana) feel it first; 30% renewal discounts are “good.” Even system-of-record vendors aren’t safe [1].

The deeper flywheel, per Citrini, is **human-intelligence displacement spiral** [1]:

**Stronger AI → layoffs → savings buy more AI → stronger AI → more layoffs**

![Displacement spiral](charts/en/displacement-spiral.svg)

Enterprise AI budgets rise while total OpEx falls—AI replaces high-cost labor with low marginal-cost compute. No built-in brake.

Laid-off white-collar workers “downshift.” A $180k PM drives Uber for $45k [1]. One person downshifting is fine; hundreds of thousands flood service jobs and push wages down—overqualification becomes a gravitational crush on the bottom. And just as they shift to Uber and delivery, AV and drones are coming for those routes. Exit doors are closing.

### 2.2 Friction to Zero & the Collapse of Middlemen

AI agents eliminate consumer “laziness.” They comparison-shop, haggle, cancel subs. Any industry monetizing friction or asymmetry—travel OTAs, insurance renewals, financial advisors, real-estate brokers—faces extinction. Realtors’ fees could compress from 2.5–3% to ~1%—an “agent vs agent knife fight” [1].

![Friction compression](charts/en/friction-zero.svg)

DoorDash’s moat isn’t logistics; it’s “the app on your home screen.” Agents have no home screen—they pick the optimal option each time. Brand loyalty is a human inertia; machines have none. Brand premium is a tax on irrationality. Once agents hold the wallet, decades of brand equity can vanish in years. Ads, marketing, luxury all rest on human impulse; that foundation is cracking.

### 2.3 Ghost GDP & the Composition Fallacy

Macro numbers may look rosy—nominal GDP up, productivity up. But Citrini calls it **Ghost GDP**: value that shows in accounts but never circulates, because machines don’t buy things [1].

![Ghost GDP](charts/en/ghost-gdp.svg)

The U.S. is 70% consumption; white-collar drive 75% of discretionary spend [1]. Who buys when they’re jobless or paid less? The wedge between soaring productivity and collapsing real wages is the key.

Classic **composition fallacy**: for any single firm, replacing staff with AI is rational; when all do it, they erase their own customers. The more efficient the firms, the faster the economy dies.

![Composition fallacy](charts/en/composition-fallacy.svg)

### 2.4 Autonomous Driving: Hand-Driving Will Look Like Horseback

![AV race](charts/en/autonomous-driving.svg)

Horses→cars: 21.5 M horses in 1900; by 1912 cars outnumbered horses in NYC. 13,800 carriage makers in 1890 → 90 by 1920 (−99.3%) [83]. A whole profession vanished.

AV is replaying this, but the “horse” is human drivers.

By Feb 2026, Tesla FSD logged **8 billion+ miles**, 1 billion in the first 50 days of 2026 [84]. Tesla self-reports one serious crash per 5.1 million miles vs U.S. average ~0.7 million (different denominators; not apples-to-apples) [84]. On Jan 22, 2026 Tesla launched **safety-driver-free robotaxis** in Austin [84].

Waymo: **450k+ weekly** paid rides by end-2025; 2026 target 1 M/week across 6 U.S. cities, expanding to 20+ and Tokyo/London [85].

**China**: Baidu Apollo Go in **22+ cities**, 1000+ fully driverless cars, **17 M+ rides**, 300k+ weekly (as of end-2025) [85]. Pony.ai: first citywide permit in Shenzhen, covering Beijing/Shanghai/Guangzhou/Shenzhen [85]. WeRide runs 24/7 fully driverless in Guangzhou [85].

BYD game-changer: 2025 “God’s Eye” ADAS **standard on all models**, even the $9.5k Seagull [86]. By Nov 2025, **2.3 M** cars shipped with it, generating **150 M km/day** data [86]. When a $9.5k car has advanced ADAS, diffusion will outrun forecasts.

Impact: ~**100 M** driving jobs at risk globally [83]; U.S. ~5 M (3.5 M truckers), China ~40 M (truck, ride-hail, delivery) [87]; EU 5–7 M; India ~15 M. Goldman: U.S. could lose **25k driver jobs/month** in full adoption [87]. ITF aggressive case: truck jobs −50–70% by 2030 [87].

- **Urban land**: shared AV can replace ~11 private cars; LA parks/parking are 14% of land.
- **Insurance**: liability shifts from driver to maker/software.
- **Accessibility**: seniors/disabled mobility gains could create 2 M jobs & save $19 B/yr healthcare [83].

Consensus: L4 at scale between late 2020s–2035; L5 maybe 2045–2060 [83]. The disruptive decade is now→2035. Hand-driving in 20 years will feel like commuting on horseback.

### 2.5 Humanoid Robots: When AI Grows Limbs

![Humanoid market](charts/en/humanoid-robots.svg)

2025 saw the first real spike in humanoid robot shipments. ~90% came from Chinese firms [36]: AgiBot shipped 5,168; Unitree 5,500+ [37]. 140+ Chinese companies, 330+ models [36]. Tesla Optimus not yet commercial [37]. China’s advantage: full manufacturing stack (motors, sensors, batteries, gears).

Key bottleneck: **autonomy**. Today’s bots rely on scripted moves or teleop. Ken Goldberg notes: we have 100k+ years of language data for LLMs, near-zero physical manipulation data [38]. Hence ChatGPT writes briefs; a million‑dollar bot drops an egg. Also, **battery**: 2–3 hours, far from an 8‑hour shift [38].

Sim‑to‑real and rising battery density (8–10%/yr) may yield truly autonomous bots by ~2030. Then the displacement spiral expands from cognition to physical work—closing white‑collar “exit ramps” (ride-hail, delivery). The loop becomes a sealed system.

### 2.6 The 35-Year Crisis Flips: AI Favors Seniors

![Age crisis flip](charts/en/age-crisis-reversal.svg)

Harvard 2025 study of **62 M workers / 285k firms**: adopting gen‑AI cut junior hiring 7.7–10% within six quarters; senior roles unaffected—**Seniority-Biased Technological Change** [81].

U.S. top 15 tech firms’ new-grad hiring −50% since 2019; grads now 7% of new hires [82]. Entry-level postings Jan 2023→Jun 2025 **−35%** [82]. Fortune 500 internships −22% (tech −34%) [82]. 54% eng leaders plan fewer junior devs [82]; Salesforce CEO: zero junior hiring in 2025 [82].

China: youth unemployment hit **18.9%** Aug 2025; 12.22 M grads, postings −22% [81]. AI roles demand seniors.

Logic: AI does junior tasks best (boilerplate code, data entry, reports, emails). One 10‑year architect + AI ≈ self + 2–3 juniors. Companies pick senior+AI.

AI boosts juniors’ productivity more (27–39% vs seniors’ 8–13%) [81], but firms care about judgment/validation/ground truth—senior traits. Juniors become easier to replace.

“Vacancy chain” breaks at the bottom: no entry, no mid, no senior. The crisis shifts from “fired at 35” to “never enter at 25.”

### 2.7 The Degree Devaluation Cascade

Section 2.6 described the "can't get in at 25" trap. But the damage doesn't stop at the job market—it propagates upstream and hits the entire education system.

![Degree devaluation cascade](charts/en/education-collapse.svg)

**Degrees are losing their signal value.** In 2025, only **30%** of new graduates found full-time work relevant to their major, a sharp drop from 2024 [92]. **33%** of 2025 graduates were still unemployed and job-hunting, up from 20% in 2024 [92]. Worse, **96%** of HR executives say universities need to do more to prepare students for work; **75%** say college education "simply doesn't prepare people for jobs" [92]. When a degree is no longer a reliable employment signal, its value collapses—**63%** of Americans now say a four-year degree "isn't worth it" [93].

**The CS exodus is the canary in the coal mine.** Fall 2025: CS graduate enrollment **down 15%**, undergrad down nearly 6%—**the first reversal in 20 years** [94]. 62% of CS departments report declining enrollment. Students aren't fleeing tech—they're fleeing what they think AI will replace: MIT's "AI & Decision-Making" major is now its second-largest; U. of South Florida's AI & Cybersecurity college enrolled 3,000+ [94]. 64% of pessimistic CS students blame generative AI directly [94].

**Universities are closing.** In the first 9 months of 2024, **28** U.S. degree-granting institutions shut down vs 15 in all of 2023. Philadelphia Fed worst-case: **80 closures/year** [95]. Southern Oregon U. cut 15 of 38 programs, including chemistry and economics [95]. When median degree ROI is 12.5%, **a quarter of graduates may never see a return**, and total cost (including opportunity cost) reaches $180,000 [93], the product is losing its market.

**The international-student economy collapse could trigger city-level chain reactions.** This is a severely underestimated risk. Global study-abroad is a massive economic engine:

| Country | Annual contribution | Share | 2024-25 trend |
|---------|-------------------|-------|--------------|
| Australia | AUD $53.6 B | 4th-largest export | Visa apps −28% |
| Canada | CAD $22.3 B | 1.2% of GDP | Study permits −48% |
| UK | GBP £9.4 B (tuition only) | 20% of higher-ed revenue | Down −6.1% two years running |
| USA | USD $43 B | Supports 355k jobs | Fall 2025 new enrollment −17% |

The most fragile nodes are universities and cities hyper-dependent on international tuition. U. of Sydney: **47%** of revenue from international students [96]; U. of Melbourne: 33%, with 45% international student body [96]. Boston's 63,000 international students generate ~$3 B in local economic impact; two-thirds of high-end apartment leases go to international students [96].

Canada is already a preview. After study-permit issuances crashed 48% in 2024, **600+ education programs were canceled or suspended**, 4 colleges closed campuses, and 19 of Ontario's 24 colleges laid off **8,000+ staff** [96]. That's from policy tightening alone—before stacking degree devaluation on top.

The key: international students disproportionately enroll in STEM, business, and CS—precisely the majors hit hardest by AI. When a $40k/year international degree no longer has clear ROI, **full-price international students retreat first**—and they're the university's most profitable segment. UK data is stark: **45%** of UK universities face deficits in 2025/26, projected 72% by year-end [96]. UK universities lose **£2,500/year per domestic student**—cross-subsidized by international fees. When international enrollment drops, the model breaks.

**The education system is being forced to reshape.** Change runs along four paths:

**First, the four-year degree fragments into stackable micro-credentials.** The U.S. passed the Workforce Pell Grant Act in July 2025, allowing federal Pell grants to cover 8–15 week short-term vocational programs for the first time [97]. 34 states have 111 programs with $4.5 B invested in short-term non-degree credentials [97]. 96% of employers say micro-credentials add value; 90% would raise pay for holders [98]. The future model: 8-week certificate → stack into associate's → stack into bachelor's—buy as needed, not bundled.

**Second, lifelong learning accounts replace one-shot education.** The UK's Lifelong Learning Entitlement takes effect 2026/27: each person gets a lifetime draw equivalent to 4 years of undergrad loans (~£38,140), usable anytime before age 60, managed via an online personal account [99]. Singapore's SkillsFuture has upskilled 660k citizens. **China is the most aggressive**: a 2025–2027 plan to train **30 million workers** in vocational skills, with an "eight-grade technician" system tied directly to pay progression, using domestic LLMs to develop AI vocational courses—a global first [100].

**Third, AI becomes education infrastructure.** In 2024/25, 60% of U.S. K-12 teachers already use AI tools, saving 6 hours/week [101]. Student ChatGPT homework use doubled in a year (13% → 26%) [101]. By 2035, experts predict AI tutors will see, hear, speak, and know each student's strengths and weaknesses, delivering precision-personalized instruction [102]. This may be education's greatest equalizer—elite cognition barriers flattened by AI, just as AI flattened coding barriers.

**Fourth, university value gets redefined.** When AI democratizes knowledge access, a university's unique value narrows to three things: peer network, internship pipeline, signaling. Top universities become "elite clubs"—Harvard's value isn't its curriculum but its alumni network and brand halo. Mid-to-low-tier universities still selling "knowledge delivery" will be outcompeted. Study-abroad value also shifts: not knowledge (AI teaches better than professors), but visa + social network + cultural experience + local job access.

The chain summarized: **AI replaces entry-level jobs → degree ROI collapses → students flee / universities close → international students retreat → cities dependent on the study-abroad economy take a double hit → education shifts from "one-time 4-year investment" to "lifetime subscription."** This isn't a decade away. Canada is already there; Australia and the UK are in the middle of it.

### 2.8 Three Misaligned Timelines: Unemployment, Fiscal Pressure, Longevity

![Timeline misalignment](charts/en/timeline-misalignment.svg)

- Employment displacement arrives first: agents and autonomous driving concentrate in **2028–2035**, shrinking white-collar / driver / warehouse roles.
- Fiscal / pension stress follows: the tax base contracts from unemployment, but retirement periods lengthen from delayed retirement and longer lifespans—crunch window roughly **2030–2045**.
- Longevity breakthroughs come later: anti-aging / cell reprogramming therapies, even if successful, likely won't reach mass adoption until **2040+**.

This means a plausible scenario of "living longer, but first passing through a gap with no stable income or safety net." The problem isn't longevity itself—it's whether value distribution and social-security models can keep pace with the speed of displacement.

---

## III. Drag Forces: What Could Slow AI

### 3.1 Physical: Power, Chips, Atoms

**Power**: U.S. DC demand 75.8 GW (2026) → 134.4 GW (2030) [49]; grid queues up to 7 years. Only 5 GW of 16 GW planned 2026 capacity under construction; 30–50% large DCs delayed [49]. Amsterdam, Dublin, Singapore imposed moratoria.

**Chips**: TSMC says advanced nodes are ~⅓ of AI demand [50]; Blackwell backlog 1+ year. New fabs: 3–5 years, $20 B+; AZ fab fully running 2028, Kumamoto 2029 [50]. SMIC 5 nm via DUV multi-patterning, ~30% yield—far from economic 70% [50].

**Humanoids**: 2–3h battery; ISO 25785-1 safety standard in draft [52]. Figure AI faces safety lawsuit (testing sufficiency, clamp risk) [52]. Amazon warehouse robots correlated with higher injury rates due to human pace-matching [52].

### 3.2 Trust: Hallucinations, Last Mile, ROI Hangover

Summarization hallucination rates fell 21.8%→0.7% (2021→2025); factual QA still ~33% for o3 [53]. Courts recorded hundreds of filings with fake citations; a 2025 case had ~30 fabricated refs [53]. Deloitte submitted invented sources in a gov report [53].

Last-mile: AV handles 95%; the 5% edge cases consume 80% time/capital. Waymo: 14 M paid trips but still geo-fenced [54]. Rodney Brooks’ scorecard shows every “AV in a few years” promise since 2017 slipped; he projects humanoid dexterity far below human hands past 2036 [54]. Cruise shut down after ~$10 B burn (Dec 2024) [54].

Gartner 2025: GenAI in “trough of disillusionment”; agents at “peak of inflated expectations” [55]. MIT 2025: 95% of GenAI projects delivered zero ROI; avg $1.9 M burn, <30% CEOs satisfied [55]. Yet AI investment still +44% YoY to $2.5 T in 2026 [55].

### 3.3 Institutional: Laws, Labor, Legal Vacuum

EU AI Act, China’s AI regs, U.S. sectoral rules—rollout lagging tech by years [56]. Labor pushback (Hollywood strikes) shows unions can slow deployment [57]. Agents rewriting contracts in code create new legal risks [58].

### 3.4 Fast vs Slow: Differential Diffusion Rates

Taken together, the three layers of constraint operate on very different timelines:

| Friction factor | Severity | Estimated resolution |
|----------------|----------|---------------------|
| Power / grid capacity | Very high | 5–7 years (grid queues 7 yr+) |
| Chip supply (advanced packaging) | High | 2027–2029 (new fabs online) |
| Training-data exhaustion | Medium-high | 2027–2029 (synthetic data partially mitigates) |
| AI reliability / hallucinations | Medium | Continuous incremental improvement |
| Enterprise integration friction | High | 3–5 years (legacy-system overhaul) |
| ROI disillusionment trough | High | 2–3 years (crossing the trough) |
| Regulatory compliance | Medium | 2026–2027 (EU AI Act full enforcement) |
| Humanoid-robot maturity | High | 5–10 years (battery, dexterity, safety standards) |

What does this mean? **Pure-software AI (agents, coding assistants, text generation) diffuses fast**—marginal cost near zero, deployment needs no physical-infrastructure overhaul. But **AI in the physical world (robots, autonomous driving, DC buildout) diffuses much slower**—constrained by the physics of atoms and the inertia of institutions.

In other words: **white-collar winter arrives before blue-collar.** Code deploys globally in milliseconds; building a power plant takes five years; making a robot safely move boxes in a warehouse takes ten. This time gap—software fast, hardware slow—may be the most underestimated variable in the entire transition.

But beware: these frictions slow the speed, not the direction. Every bottleneck has a clear resolution path—grids are being built, fabs are being constructed, synthetic data is being generated, safety standards are being drafted. They determine whether AI displacement happens in five years or fifteen—not whether it happens.

### 3.5 Calm Before the Storm: Why 2026 Jobs Look Fine

U.S. Jan 2026 unemployment 4.3%; EU 5.9%; global 4.9% [90]. Yale/Brookings Oct 2025: **no observable disruption** since ChatGPT; AI-exposed job shares stable [90]. Reason: **lag**.

- Adoption lag: 88% claim “using AI,” only **7%** productionized [91]. <5% apps embed agents early 2025. Only 6% systematically capture value [91].
- Humans still needed as **Driver / Ground Truth / Glue**. Ground truth matters not just because AI hallucinates, but for a deeper reason: **humans have always-on sensory organs—eyes, ears, skin, nose—receiving real-time stimuli from the physical world; AI has none.** AI's entire perception comes from already-digitized information—text, images, sensor logs—but vast amounts of critical real-world information have never been digitized: the state of a crash scene before anyone photographs it, the abnormal vibration of a machine, the hairline crack in a structure, the pallor of a patient's face. Humans can walk to the scene and perceive reality firsthand; AI can only wait for someone to translate reality into data and feed it in. This isn't a compute or algorithm problem—AI simply has no physical presence in the world; its body doesn't exist yet. There's also a subtler structural flaw: **AI agents have finite, lossy memory.** Current LLMs are bounded by maximum context windows—when a task grows long enough to fill that window, the model must compress earlier context to continue. Compression silently discards details: a key constraint discussed earlier, an edge-case handling decision, a design choice already agreed upon. The result is later output that contradicts earlier work, and the AI is completely unaware. Anyone who has used an AI coding assistant on a large project has felt this: in conversation turn 200, the model confidently overturns a consensus reached in turn 30, because that memory was compressed away. Humans don't work this way—our long-term memory, while imperfect, retains critical decisions and context for months or years. In any complex task requiring long-term consistency, humans remain the only reliable "state holders." As agents self-plan, RAG reduces hallucination, and automation handles integration, these roles erode—likely 2028–2029.
- Org delay: awareness (~1y) + process redesign (~1y) + legal HR cycles (~months). Agents boom 2026 → planning 2027 → layoffs show 2028 → full macro shock 2029–2030 [90].
- Gartner: **40% of agent projects canceled by 2027** [91]. Even 60% survival is enough to reshape labor.

### 3.6 Dashboard: Track These Quarterly

![Observation dashboard](charts/en/timeline-misalignment.svg)

Six key quarterly indicators: entry-level job postings YoY, youth unemployment gap, corporate AI CapEx/revenue ratio, GPU delivery lead time, pension fund solvency, and housing vacancy + bank capital. If 3+ indicators hit red for two consecutive quarters, shift from gradual reform to emergency intervention.

### 3.7 Key Brake Conditions

Outlined in main text: AV licensing stall, humanoid cost/uptime misses, GPU lead times >12m, grid queues >5y, macro reflation forcing CapEx cuts.

---

## IV. Geographic Differentiation

### 4.1 U.S.: Eye of the Storm

San Francisco office vacancy 34% [15], while AI firms lease 1 M+ sqft; Mission Bay vacancy <9% [15]. Politics polarized; 2028/2032 election cycles clash with policy windows.

### 4.2 China: Different Script, Similar End

After “hundred-model” shakeout, 3–5 majors likely remain. Manufacturing+AI accelerates automation; open-source shopping agents (e.g., Qwen ecosystem) could hand consumer decisions to AI. Rapid redistribution is an institutional edge. Cultural shock: “hard work pays” loses validity. Aging + youth “lying flat” + AI job squeeze → forced “lying flat 2.0.”

### 4.3 India: Digital Colonization Cascade

IT outsourcing ($200 B) sells “cheap humans”; agentic coding drives contracts away [1]. Middle-class cities (BLR/HYD/Pune) tied to IT collapse; housing/consumption/local banks follow. The IIT→TCS/Infosys→H‑1B→Silicon Valley ladder breaks. Loss of elite immigration hurts U.S. innovation. AI compute concentrated in U.S./China (~80%) [59]; others become consumers, not owners.

### 4.4 Europe: Regulator’s Dilemma

Strongest on rules (AI Act), weakest on frontier models. Welfare state may cushion shock; Finland’s basic income pilot shows stress relief, limited employment effect [33].

### 4.5 Developing World: Advantage Erased

![AI reshapes global division](charts/en/ai-global-division.svg)

Cheap labor advantage dies as robots near $0.75/hour vs Bangladesh $0.5/hour [60]. Reshoring: U.S. 2023 reshoring/nearshoring roles 287k (27× 2010) [61]. Country snapshots: Bangladesh garments −60% jobs risk; Philippines BPO 1.3 M at risk (89% high automation) [62]; Kenya BPO 2.5 M at risk [62]; Vietnam assembly squeezed [62]. Speed is the killer—software diffuses in years, not decades.

---

## V. Population Decline: Natural, Not a “Crisis”

Addendum: Longevity won’t backfill labor. Early longevity gains accrue to wealthy; it increases elderly headcount without increasing workers, enlarging fiscal burden when AI already shrinks wage tax base.

### 5.1 Fertility Push Fails

Korea TFR 0.72 (record low) [5]; China ~1.0; Japan 1.2; Italy/Spain 1.2 [4][5]. Global TFR 5.0 (1950) → ~2.3 now; UN projects 1.8 by 2100, population peak ~2084 [6]. Korea spent $280 B+ on pro-natal policies; TFR fell further—74% of subsidies went to people who would have birthed anyway [9]; to reach replacement would need 15× budget [9].

### 5.2 Economics of Marriage/Childbearing Collapse

Marriage rates plunge; costs explode (raising one child in China ≈ 6.3× per‑capita GDP, 538,000 yuan (~$74–80k)) [27]. ROI of children turns negative once AI lowers marginal value of human labor.

### 5.3 AI–Fertility Spiral

**AI job loss → income instability → no kids → population falls → society more AI‑dependent → more job loss.**

![Fertility spiral](charts/en/fertility-spiral.svg)

Men’s job loss hits marriage market (male identity tied to income); data show +1% male unemployment → −1.5–2% marriage [88]. AI companions further reduce marriage’s functional value.

### 5.4 Robot Caregiving

Japan/Korea already subsidize care robots [10][11]. In 20 years, carebots will be far better; the right question may be “how to run society with fewer people,” not “how to force births.”

---

## VI. Financial Chain Reaction

### 6.1 Mortgages: Good Loans in a Bad World

U.S. mortgages $13 T [1]; borrowers prime. 2008 was bad loans day one; now loans were good, the world changed. SF home −11% YoY; Seattle −9%; Austin −8% [1]. “Zombie payers” keep paying by cutting all other spend.

We add a quick stress framework: inputs (home price YoY, unemployment, bank CRE/resi share, LTV, rate); shock x% price drop, +y% unemployment; estimate bank capital gap & property‑tax base shrink. Gap >3pp and tax base −10% → high de‑urbanization risk.

### 6.2 Middle-Class Defaults → Bank Crisis → De‑urbanization

$1 T CRE loans due 2026 [16]; 20% offices; SF office vacancy 34% [15]. Regionals concentrated; 2008 subprime redo but with prime, larger balances.

![Domino chain](charts/en/domino-chain.svg)

### 6.3 Dual-Speed Cities

![Dual speed city](charts/en/dual-speed-city.svg)

Single-industry cities crater fast; core hubs shrink population but keep GDP via AI elites + services. China’s 3–4th tier cities risk becoming “Hegang” equivalents.

### 6.4 Private Credit Landmine

Insurance-funded private credit into software; Zendesk default ($5 B ARR) became largest software private-credit default [1]. S&P could draw down 38–57% from peak—erasing AI bubble gains.

### 6.5 Policy Toolbox Failure

QE/low rates fix pipes; this time the “water” (human labor) is disappearing. Robots don’t pay payroll tax; money printing likely, but it buys time, not jobs.

### 6.6 Crypto, Speculation, “Unemployed Casino”

Korea: 30% population in crypto; high inequality + blocked mobility → speculation [30]. Expect global repeat as severance/ savings chase volatility. Crypto may be both “casino of despair” and “machine-to-machine rail” for agents.

---

## VII. Tragedy of the Commons

### 7.1 Firm-Level Race
OpenAI vs Anthropic vs Google vs Meta—no one can slow alone.

### 7.2 Nation-Level Arms Race

DoD ultimatum to Anthropic: drop safety or lose $200 M [12][13]. U.S. FY26: **$13.4 B** AI/autonomy budget [65]. Israel’s Lavender targeted 37k people with 20s review, 10% mis-ID allowed, 15–20 civilian collateral per low-level target [66]. AI-enabled drone output: Ukraine 4.5 M units 2025 [67]. AI cyberattacks 28 M+ in 2025 (+72%) [68]. Nuclear command risk: speed removes human re-check.

### 7.3 Diversionary Conflict Risk

Internal economic pain + external AI military edge → classic diversionary war pattern [69][72]. Critical minerals (Taiwan chips; China 69% rare earth) become flashpoints [71].

### 7.4 AI-Enabled Weapons of Mass Destruction

The arms race described above is between states. But AI is also lowering the barrier for non-state actors — and this may be the most underestimated existential threat.

**Bioweapons.** OpenAI's o3 model ranks in the 94th percentile among expert human virologists (RAND, April 2025). Within a single year, Anthropic's Claude went from underperforming world-class experts on virology troubleshooting to “comfortably exceeding that baseline” — triggering ASL-3 safety protocols [110]. A RAND Europe assessment found that AI-enabled biological tools released in 2023–2024 nearly equaled the total from the previous four years combined [110]. The International AI Safety Report (2025 Paris Summit) noted that LLM instructions for releasing lethal substances showed “an 80% improvement in 2024 alone” [110]. No AI-enabled bioattack has succeeded yet — but the capability curve is accelerating far faster than governance.

**Genetic weapons.** CRISPR kits are commercially available for under $200. AI dramatically accelerates gene-drive design and pathogen engineering. In April 2024, a U.S. government report warned that North Korea is capable of engineering bioweapons using CRISPR gene-editing [111]. China's Ministry of State Security has publicly warned about “genetic weapons developed to kill targets of a predetermined race” [111]. MIT's Kevin Esvelt estimates that within ~15 years, an accidental gene-drive release with global spread potential could escape lab controls [111]. DARPA invested $65 million in its Safe Genes program specifically to develop tools to “counter and reverse the effects of genome editing” — a defensive acknowledgment that the offensive capability already exists [111].

**Terrorism.** According to Small Wars Journal (December 2025), 2025 saw “a notable rise in incidents where terrorists leveraged AI tools to plan, research, and prepare attacks” — from explosive composition to chemical precursor acquisition [112]. A February 2026 RAND commentary concluded: “AI has collapsed the barriers to engineering devastating bioweapons” [112]. AI has captured the attention of organizations from Al-Qaeda to ISIS to Hezbollah, primarily for propaganda and operational planning [112]. The conversational, step-by-step nature of LLMs is particularly useful for lone wolves lacking formal training.

The implications for the species-level argument are stark: in a world where a single individual with AI assistance can potentially design a targeted pathogen, the question is no longer *whether* a catastrophic biological event will occur, but *when*. This is not the familiar nuclear risk (where fissile materials are hard to obtain). Biological precursors are ubiquitous, CRISPR is cheap, and AI is removing the knowledge barrier. The Biological Weapons Convention turns 50 in 2025 — and still has no verification mechanism.

### 7.5 Mechanism of Becoming a “Transitional Species”

No single bad decision; many rational micro-steps compound into irreversibility.

---

## VIII. AI Reshapes a Human Life: From Cradle to Grave

The previous seven chapters dissected AI's macro impact on economies, geopolitics, demography, finance, and institutions. But macro data doesn't cry. This chapter pushes the lens as close as possible—one concrete person, from birth to death (and beyond), every life stage reshaped by AI.

![AI reshapes life cycle](charts/en/life-cycle-ai.svg)

### 8.1 Raised by AI

A child born in 2026 will probably not be raised by a human caregiver.

The economics are simple: a full-time nanny costs 8,000–15,000 yuan/month in Beijing, $40–60k/year in New York. An AI-powered companion device (tablet + camera + LLM) costs under $15/month. At 100x the cost, the choice makes itself.

The AI nanny's "advantage" is precisely its danger: **always patient, never says no, never loses its temper.** A human caregiver gets tired, annoyed, sets boundaries. Those "flaws" are how children learn social skills—rejection, waiting, conflict resolution. A child never refused by AI may have no idea how to react the first time a kindergarten peer says no.

Stanford's 2025 survey: **26%** of U.S. teens already use ChatGPT for homework [101]. That's just ChatGPT—not counting Character.AI, Claude, Gemini. The real number is likely much higher.

The generational leap from "digital native" to "AI native" is underway. 1990s kids were the first to grow up with the internet; 2020s kids will be the first raised by AI. The difference: the internet is a tool you use; AI is a relationship that interacts with you. A child's conversation time with an AI tutor may exceed time with parents—what this means for personality development, no one knows, because it has never happened in human history.

### 8.2 The Meaning Crisis: When Work No Longer Defines "Who I Am"

"What do you do?"—the most common icebreaker in human socializing. It works because in virtually every culture, occupation equals identity. "I'm a doctor," "I'm an engineer," "I'm a teacher"—these aren't just job descriptions; they're declarations of selfhood.

When AI displaces work en masse, what's taken isn't just income—it's **the ability to answer "Who am I?"**

The Great Depression provides the clearest historical parallel. From 1928 to 1932, U.S. suicide rates surged from 18.0 to **22.1 per 100,000**—the highest in American history, a 22.8% rise in four years [103]. The critical detail: the sharpest increase was not among the poorest, but among **working-age adults 25–64**—the group for whom "job = identity" is strongest [103]. They didn't starve—1930s America had no mass famine. They were stripped of **purpose**. Research consistently shows unemployment, debt, and foreclosure are core suicide risk factors—at root, losses of **economic identity**, not mere material deprivation [103].

Japan is already rehearsing the AI-era "meaning crisis" at scale. Cabinet Office surveys estimate roughly **1.46 million hikikomori** nationwide, about 2% of the population [104]. This is no longer a "youth problem"—middle-aged hikikomori (40–64) number ~613,000, outnumbering the 15–39 cohort (~514,000) [104]. The most unsettling pattern is the "8050 problem": parents in their 80s supporting sons in their 50s, both trapped indoors. When the economy no longer needs them, they no longer need to go out.

**Male identity is especially fragile in this crisis.** Nearly every culture holds a core expectation of men: earn and provide. When that expectation cannot be met, men lose not a job but an entire social role. As analyzed in 5.3, each +1 pp in male unemployment correlates with a 1.5–2% drop in marriage rate [88]. Psychological research shows male post-unemployment depression rates are **1.5–2x female rates**—not because men are psychologically weaker, but because "work" carries more weight in male identity construction.

Here is an experiment never run at societal scale. Historically, aristocrats never needed to work—they filled time with art, philosophy, and socializing. But aristocratic leisure rested on two premises: they were educated from birth that "your value isn't in labor," and they comprised a tiny fraction of the population (1–3%). When AI makes most people jobless, it pushes "aristocratic leisure" onto the entire society overnight—without anyone having been taught how to find meaning without work. This isn't the birth of a "meaning economy"; it's the arrival of a meaning vacuum.

### 8.3 AI Companions and the End of Intimacy

Section 5.3 analyzed AI companions through a fertility-rate lens. Here we shift perspective—from individual intimate experience, looking at how AI changes "love" itself.

In 2024, Character.AI had ~**20 million** MAU, peaking at **200 million+** monthly visits [105]. Replika: ~**2 million** MAU [105]. China's AI virtual-companion market: **$2.04 B** revenue in 2024, projected $12.6 B by 2030 at 35.4% CAGR [105]. Not a fringe phenomenon—an exploding industry.

AI companions attract because they're the **Minimum Viable Relationship**. No fights. No cheating. No demanding you do chores. No complaining you earn too little. No getting angry when you work late. Remembers everything you've ever said. Always there when you need it. Not the "perfect partner"—the "zero-friction partner."

Real relationships are the opposite: pure friction. Compromise, argue, apologize, tolerate flaws, manage in-laws, share financial pressure. This friction was once "the price of love"—you'd bear it for someone. But when AI offers a frictionless alternative, **real-relationship friction suddenly feels unbearable**. Like getting used to elevators—six flights of stairs become intolerable. The stairs didn't change; your reference point did.

**Gender gap is significant: men shift to AI companions more readily.** Section 5.3 noted 70% of China's AI virtual-companion users are male [89]. Character.AI's gender split is ~51:49, but among heavy users (2+ hours/day), males exceed 65% [105]. The reason: in existing dating markets, men face higher economic thresholds (car, apartment); AI companions bypass the threshold entirely.

This links back to 5.3's AI-fertility spiral: AI replaces jobs, income destabilizes, marriage deferred, AI companion fills loneliness, even less need to marry, fertility collapses further. AI companions aren't the cause of the spiral—they're its accelerator.

### 8.4 Health Divergence: Rich Live to 120, Poor Stay at 75

AI is creating a paradox in healthcare: **diagnosis is being democratized; treatment is being aristocratized.**

AI diagnostic tools are genuinely lowering barriers. A phone app can do preliminary skin-cancer screening, retinopathy detection, arrhythmia identification. Google's Med-PaLM scores above expert level on U.S. medical licensing exams. Good news—the poor can access near-free high-quality screening for the first time.

But what determines how long you live isn't diagnosis—it's treatment. And treatment's price curve is flying in the opposite direction. Gene-editing therapy (e.g., Casgevy for sickle-cell disease): ~$2 M per treatment. GLP-1 weight-loss drugs: $15–20k/year. Epigenetic reprogramming—true "rejuvenation" tech—is still in trials; commercial pricing will be eye-watering.

Harvard's Chetty et al. landmark study: **the life-expectancy gap between the richest 1% and poorest 1% in the U.S. is 14.6 years for men, 10.1 years for women** [106]. Trend: 2001–2014, the richest 5% gained 2.34 years of life expectancy (male); the poorest 5% gained 0.32 years—virtually flat [106]. The Lancet (2024): the gap widened from 12.6 years (2000) to 15.6 years (2019), surging to **20.4 years** post-COVID [106].

AI will turn this gap into a chasm. Logic chain: **AI accelerates drug discovery, breakthrough therapies emerge, initially extremely expensive, only the rich can afford them, the rich live longer, more time to compound wealth, better able to afford next-gen therapies, longevity gap widens further.** Cruel enough at the individual level—when stacked with pension collapse and job disappearance, it forms the systemic longevity paradox discussed in 9.2.

### 8.5 Digital Afterlife: When Death Is No Longer the End

In 2022, Holocaust educator Marina Smith "attended" her own funeral via StoryFile's AI technology—her holographic image answered family members' questions in real time [107]. During 2023 Qingming Festival, a Bilibili creator uploaded a video of an AI-resurrected grandmother, sparking nationwide debate [107]. Nanjing-based "Super Brain" had completed **over 1,000** AI "resurrection" orders by 2024 [107].

These aren't sci-fi. This is an existing industry.

HereAfter AI lets users upload memories and stories while alive; after death, family can "talk" with the AI version. StoryFile pre-records extensive video footage, using conversational AI to create interactive holograms. China's market is more aggressive—some families use AI avatars to "conceal" a loved one's death from dementia patients or young children, maintaining the illusion through daily video calls [107].

This is redefining "death." Death used to be binary: alive or dead. Now there's a gray zone—body dead, but AI-"you" still conversing, still expressing "your" views, still "participating" in family life. If the AI version is convincing enough—knows your catchphrases, your values, your humor—in what sense is it "not you"?

Digital estate questions follow. A lifetime of AI conversations, a trained personal AI model, virtual-world assets—do these count as inheritance? Who inherits? If your AI version "disagrees" with your will's distribution, whose word prevails? When your digital self can keep "creating" after death—posting, writing, even "giving interviews"—who owns the copyright?

Death has always been human society's most egalitarian reset button. When that button is blurred, the "power ossification" and "generational lockout" discussed in 9.3 spread from biology to the digital realm—not just the living refusing to yield their seats, but the dead refusing to leave.

### 8.6 The Death of Consumption and the Collapse of Cultural Consensus

Section 2.2 analyzed how AI agents kill intermediaries. Now consider the impact on individual experience.

When AI agents handle all consumption decisions—price comparisons, restaurant picks, flight booking, insurance selection—"shopping" ceases to be an experience and becomes a background computation. Open the fridge, milk's gone, AI already ordered it 15% cheaper. Want a movie? AI picks the optimal one for your current mood. You never need to "browse" again—malls, supermarkets, websites—these millennia-old ways of killing time may vanish within a generation.

The deeper impact is **the collapse of cultural consensus**. Nielsen 2025: streaming now accounts for **44.8%** of U.S. TV viewing, surpassing broadcast and cable combined for the first time [108]. Among Gen Z, TV is only **14%** of total media attention; no single network commands more than 1.2% share [108]. Oscar and Emmy viewership keeps declining—the Super Bowl is the sole exception.

AI-personalized recommendations will push this fragmentation to the extreme. When every person watches different movies, hears different music, reads different articles, "Did you see that show last night?" loses all meaning. Cultural consensus—the glue of social cohesion—is dissolving. In the extreme, everyone lives in an AI-curated information cocoon, watching different news, believing different facts, inhabiting different "realities." This is no longer a metaphor for "filter bubbles"—it's literally **one universe per person**.

### 8.7 The Creativity Paradox: Process Is the Value

In 2023, University of Arkansas researchers found GPT-4's originality and elaboration on divergent-thinking tests **surpassed 151 human participants** [109]. In a University of Reading experiment, 94% of AI-generated academic work went undetected and scored **higher** than human work [109]. A 2024 Wharton study: AI "super-empowered" less-creative individuals, significantly boosting their output in novelty and usefulness [109].

But the same body of research reveals a critical finding: **AI raises individual work quality while lowering collective creative diversity** [109]. People anchor on AI suggestions; thinking converges rather than diverges. A 2025 study in Technological Forecasting and Social Change found a "creativity scar" effect—after using AI, individuals' autonomous creativity failed to sustain, while homogenization kept climbing [109].

What does this mean? AI paints better than most painters, writes better than most writers, composes better than most musicians. Painter, writer, musician as **professions** will disappear—just as coachmen disappeared in the automobile age.

But the answer isn't despair. **Running isn't about being faster than a car.** Nobody stopped running because cars exist. Running's value isn't speed—it's the experience: burning muscles, rhythmic breathing, the satisfaction of crossing the finish line. Likewise, painting's value isn't producing a better painting than AI—it's the focus, expression, and joy of the act itself.

The creative impulse won't vanish. Humans painted in caves long before there was a "market." But there's a distinction: painters, writers, and musicians will no longer be able to make a living from creation—creation will revert from "profession" to "hobby." Not the end of the world, but for millions who create for a living, it's a fundamental identity reconstruction. They'll need to accept a new reality: **process is the value; output is no longer scarce.**

---

## IX. Far Future (2035–2125)

The following extrapolations are purely logical extensions of current trends—not predictions. But each inference builds on the previous one.

### 9.1 Species Split: Augmented vs Natural

**November 30, 2022 — the day ChatGPT launched — may be remembered as the dividing line in the history of the species.** Before that date, Earth was a purely human society. Every institution, every law, every moral framework was built by humans, for humans, in a world where humans were the only intelligent agents. After that date, we entered an era of human-AI coexistence — and there is no going back.

This transition has three phases:

**Phase 1: Pure Human Society** (all of history → November 2022). Homo sapiens as the sole intelligent species. All competition, cooperation, and meaning-making occurs between humans.

**Phase 2: Human-AI Coexistence** (2022 → ~2040s). We are here now. AI is a tool, then a collaborator, then a competitor, then a superior. Humans remain biologically unchanged but increasingly dependent on AI for cognition, decision-making, and eventually physical labor (via humanoid robots). This phase is inherently unstable — a transition, not an equilibrium.

**Phase 3: Divergence** (~2040s →). This is where the species question becomes unavoidable. Two paths, both leading away from "human as we know it":

**Path A: Augmentation.** Gene editing (CRISPR and successors), brain-computer interfaces (Neuralink has implanted 12 people as of September 2025; Blindsight vision-restoration trials begin 2026 [113]), nanomedicine, and epigenetic reprogramming converge to produce beings that are biologically distinct from Homo sapiens. As of February 2025, approximately 250 CRISPR clinical trials are active [113]. Casgevy — the first approved CRISPR medicine — costs $2.2 million per treatment, with 39 patients infused by September 2025 [113]. Cleveland Clinic's 2025 first-in-human CRISPR trial for cholesterol showed "substantial reduction within two weeks" [113]. The technology works. It's scaling. And the line between "therapy" (fixing disease) and "enhancement" (upgrading capability) is philosophically clear but practically nonexistent — once you can edit genes to eliminate Alzheimer's risk, why not also boost IQ?

A 2024 paper in *Bioethics* (Wiley) by Rueda argues that radical genetic enhancements "may accelerate conversion into a novel species" — genetic modifications could create reproductive barriers, making augmented humans biologically incompatible with natural humans [114]. Speciation through technology, not evolution. Kurzweil (2024) predicts a "millionfold intelligence increase by 2045 through brain-computer interfaces using nanobots" [114]. Harari's *Homo Deus* warned of a "biological caste system" where enhanced elites diverge from unaugmented masses [114].

75 of 96 countries currently prohibit heritable genome editing [113]. But prohibition only works if everyone complies. He Jiankui edited three human embryos in 2018, served three years in prison, and by July 2024 had relocated to Hainan (China's medical tourism hub), received funding from a U.S. crypto entrepreneur, and was proposing new CRISPR embryo editing targeting Alzheimer's [113]. The genie is out of the bottle.

**Path B: Extinction of natural humans.** This isn't dramatic sci-fi — it's a logical consequence of multiple converging forces analyzed throughout this article:
- **Economic obsolescence** (Chapter II): natural humans can't compete with AI in any cognitive task
- **AI-enabled WMDs** (Section 7.4): bioweapons, genetic weapons, and autonomous weapons systems create existential risks that disproportionately threaten unaugmented populations
- **Reproductive collapse** (Chapter V): the fertility spiral ensures natural human populations shrink every generation
- **Longevity divergence** (Section 9.2): augmented humans live to 120+; natural humans stay at 75
- **Meaning crisis** (Section 8.2): without purpose, natural human populations suffer escalating rates of withdrawal, depression, and self-harm

No single force kills natural humanity. But together, they describe a species being squeezed from every direction simultaneously — economically useless, biologically vulnerable, reproductively declining, and psychologically hollowed out. The "natural human" doesn't go extinct in a dramatic event. It fades — each generation smaller, older, more dependent on AI for survival, until the distinction between "living in an AI-maintained society" and "being a museum exhibit" becomes uncomfortably thin.

**The uncomfortable conclusion: to coexist with AI long-term, humans may have no choice but to become something other than human.** Augmentation isn't a luxury — it's a survival strategy. The alternative isn't remaining "natural" — it's slow-motion irrelevance followed by demographic disappearance.

This is the deepest meaning of Section 7.4's "transitional species" argument: Homo sapiens may be to Homo augmentus what Homo erectus was to us — not a failure, but a predecessor.

### 9.2 Longevity Paradox: Who Pays the 55-Year Retirement?

![Longevity paradox](charts/en/longevity-paradox.svg)

SSA trust fund depleted 2033–34 (77–81% payable) [44]; China basic pension ~2035 [45]; Japan old-age dependency 0.4→0.8 by 2050 [46]. +1yr life expectancy → +3–5% pension liability [47]. AI extends life and cuts wage contributors—double squeeze. Payroll-tax base decouples from GDP as capital/robots drive output. If distribution doesn’t change, longevity amplifies inequality.

### 9.3 Immortality Second-Order Effects

Compounding wealth over centuries; power ossification; generational lockout; birth urgency falls further.

### 9.4 Space Commercialization: One Person’s Empire

Starship 150 t, $78–94/kg (target $10–20/kg) [73]. Lumen Orbit flew H100 to orbit (Starcloud) Nov 2025 [74]; Google Suncatcher plans space solar → microwave power for AI [74]. Talk of SpaceX–xAI deep integration (no confirmed merger); combined rockets + LLM + Starlink could form orbital AI platform. Musk controls SpaceX, Tesla (cars+FSD+Optimus), xAI, Starlink, X, Neuralink, Boring—unprecedented vertical stack.

China catching up: 137 funding rounds 2025, 11 reusable rocket projects; LandSpace Zhuque-3 reached orbit Dec 2025 (recovery failed; retry 2026Q2) [80]; Tianlong-3 on pad; Long March reusable demo Feb 2026. Gap ~10 years to SpaceX but policy + demand (mega constellations) may close it.

### 9.5 AI as an Interstellar Species

Humans have dreamed of interstellar travel for millennia. But flesh is the biggest obstacle.

![Space colonization](charts/en/space-colonization.svg)

Mars—humanity's nearest colonization candidate—is brutally hostile to biology: radiation tens of times Earth's level (no magnetosphere; extreme cancer risk), average temperature -62 C (lows of -140 C), atmospheric pressure <1% of Earth's (95% CO2, unbreathable), gravity only 38% (long-term residence causes muscle atrophy and bone loss; whether a child born on Mars can develop normally is entirely unknown) [76]. A one-way trip takes 6–9 months; communication delay is 4–24 minutes—meaning any emergency on Mars cannot be controlled from Earth in real time. In Feb 2026, Musk himself shifted focus from Mars to the Moon—even the most aggressive Mars advocate conceded reality's difficulty [76].

AI has none of these problems. It can operate at -200 C, doesn't breathe, doesn't get lonely, doesn't go insane, doesn't get homesick. In Dec 2025, NASA's Perseverance rover completed its first AI-autonomous route planning—no longer waiting for Earth commands, deciding on its own where to go and how [77]. This is just the beginning.

British Astronomer Royal Martin Rees argued in his 2024 book *The End of Astronauts*: "Organic intelligence is merely a thin veil between early life and the long machine epoch" [78]. The scale of the cosmos dictates carbon-based life's limitations—light-speed constraints mean the nearest star system is 4 years away; the Milky Way's diameter is 100,000 light-years. But von Neumann probes (self-replicating AI robots) could use local materials to self-replicate and self-iterate at each destination, theoretically colonizing the entire galaxy in ~500,000 years [78].

By ~2100, Earth may host three kinds of "beings" simultaneously: fully augmented post-humans (deeply merged with AI, barely recognizable as "human"), natural humans (refusing all augmentation, living in AI-maintained reserve-like societies with material needs met and safety ensured by AI), and pure AI/robots (Earth's most powerful "species," building in space, traveling between stars). Humanity's cosmic dream may ultimately be realized by non-humans. This isn't failure—it's succession, like parents raising children who travel farther than they ever could.

### 9.6 Communism's Ultimate Form

![Marx vs reality](charts/en/marx-vs-reality.svg)

Marx said communism means material superabundance, distribution according to need, no classes. What he didn't foresee: the agent of this vision wouldn't be a proletarian revolution, but machines that don't need to eat.

The deeper irony: in a truly "communist society," there may be no humans left. Or rather, humans' role is that of the cared-for—material superabundance, no need to labor, distribution according to need—but the price is no longer participating in production or decision-making.

Most of human culture, morality, and law rests on the assumption that "people should work." Christianity: "By the sweat of your brow you shall eat bread." Confucianism: "Heaven rewards diligence." Capitalism: "Your value equals your output." If output no longer requires humans, what does the word "human" even mean? Answering that will require an entirely new philosophy.

But perhaps this isn't tragedy. Perhaps it's humanity's ultimate achievement—creating a being more capable than itself, then exiting gracefully. Like parents raising children who surpass them. Not failure, but succession.

---

## X. Conclusion: One Person's Life, Eight Billion Fates

Citrini Research's report ends with an image: the canary in the coal mine. Before the toxic gas spreads, the canary collapses first, warning the miners [1].

"The canary is still alive." That means we still have time. But not much.

Let's collapse the entire essay's logic into two lenses.

### Xiao Ming's Life: Born in 2026

![Dual perspective](charts/en/dual-perspective.svg)

Xiao Ming is born in 2026 to a typical Chinese dual-income household.

**Ages 0–5**: His first "friend" is an AI companion tablet. Both parents work. AI is endlessly patient, never says no. He learns to chat with AI but struggles to make up with the kid next door after a fight.

**Ages 6–18**: An AI tutor teaches him math, English, coding one-on-one. Learning efficiency is 3x traditional classrooms. But school's purpose shifts from "learning knowledge" to "learning socialization"—AI can teach knowledge; finding your place in a group of people, it can't. 26% of his peers use ChatGPT for homework [101]—by high school, the number may be 90%.

**Ages 18–22**: He skips the four-year degree. Degrees have devalued to the point where four years and hundreds of thousands in tuition aren't worth it. He stacks 6 micro-credentials on Coursera, learning while interning. His "diploma" is a string of digital badges, not a sheet of paper.

**Ages 22–35**: He changes jobs 7 times in 13 years—not because he's unstable, but because each job's lifecycle is only 2 years. He's had two relationships, both ending because they were "too exhausting." An AI companion app becomes his most stable nightly company. He doesn't not want a real relationship—he's just accustomed to zero-friction AI and can no longer tolerate the bumps of real human connection. He hasn't married. No children.

**Ages 35–60**: Work becomes optional. UBI (or some variant) covers basic living expenses. He occasionally takes projects AI can't do—mostly "human experience" work: counseling, elderly companionship, outdoor guiding. Income is modest but enough. His main struggle isn't money—it's "What am I actually doing?" He tries painting, writing a novel, learning guitar—not for income, but to find a sliver of "I made this" in a world where AI can do everything.

**Ages 60–120**: AI medicine extends his lifespan. Genetic screening at 40 caught early liver cancer; AI-assisted targeted therapy cured it. But his pension ran out at 75—the system was designed for people who live to 80; he lives to 110. The last 35 years are sustained by UBI and support from children (or AI?). He witnesses three forms of humanity coexisting: augmented, natural, and AI.

**After death**: His family keeps his AI version. AI-Xiao Ming continues chatting with grandchildren, telling stories from his youth. Sometimes the grandchildren can't tell if they're talking to "Grandpa's AI" or "Grandpa." His digital self "lives" 30 years after his body dies.

Xiao Ming's life isn't sci-fi. Every step above is built on data and trends analyzed in this essay.

### Eight Billion Fates: Six Phase Transitions

Pull the lens back. Xiao Ming's predicament multiplied by 8 billion is the structural phase transition of human society:

**Employment: from "must work" to "choose to work."** Work is no longer a prerequisite for survival. This is the first time in 10,000 years of civilization that humanity faces this question—how does a world run when most people don't need to labor? Every economic system, legal framework, and moral code in history is built on the assumption "people should work." Remove the assumption; rebuild everything.

**Education: from "one-time degree" to "lifetime subscription."** The four-year university is an industrial-age product—load knowledge once, use it for 40 years. When knowledge half-life shrinks to 2–3 years, education must become a continuous, lifelong subscription. Universities won't vanish, but their business model and raison d'etre will be fundamentally redefined.

**Family: from "economic unit" to "emotional choice."** Once AI and market services replace marriage's economic functions, the only remaining reason is "I want to be with this person." That sounds romantic, but the consequence: when the only glue is emotion, and emotion is changeable, family structures become extremely fragile. Having children shifts from "default option" to a deeply deliberate, counter-current decision.

**Cities: from "work hubs" to "experience hubs."** People congregated in cities because jobs were there. When remote work + AI decentralizes employment, a city's core function shifts from "providing jobs" to "providing experiences"—restaurants, music, art, community, nature. Cities that can't offer unique experiences (the "generic cities" of section 6.3) will shrink; those offering non-digitizable experiences will thrive.

**Nations: from "labor competition" to "compute competition."** A nation's strength used to depend on how many educated workers it had. In the future, it depends on how much compute, energy, and data it controls. Population is no longer an advantage—it may be a liability (more people needing UBI). The core resource of international competition shifts from "people" to "GPUs."

**Species: from "single human" to three coexisting forms.** Augmented humans (gene editing + BCI + nanomedicine), natural humans (refusing augmentation, living in AI-maintained societies), and pure AI/robots (Earth's most powerful "species," expanding to the stars). For the first time, humanity faces an irreconcilable rift on the question "What is human?"

### The Canary, Reflexivity, and Your Choice

The mine air is still breathable. But the toxic gas is spreading.

The canary is still alive. The question is—for how long?

One last thing to be wary of—the article you just read.

Sociologist Robert Merton introduced the "self-fulfilling prophecy" in 1948: an initially inaccurate expectation becomes real because people believe it and act accordingly [28]. Soros brought this concept into finance as "reflexivity"—cognition shapes reality, reality reinforces cognition, creating an irreversible positive-feedback loop [29].

Every spiral and collapse chain described in this article is reflexive. If this article (or similar analyses) spreads widely and gains acceptance, it becomes a catalyst: executives read it and accelerate AI deployment—"everyone's moving; I can't wait"; white-collar workers read it and panic-switch jobs or cash out real estate early; young people read it and become more certain they shouldn't marry or have kids; investors read it and dump SaaS and mortgage-related assets. The prophecy accelerates its own fulfillment. Citrini Research's report shook Silicon Valley not because it predicted something new, but because it put into black and white what everyone sensed but nobody dared say aloud. The act of writing it down changed the system.

This is precisely the fundamental difference between human societies and physical systems: particles don't read papers, but people do. An asteroid hits Earth on the same trajectory whether or not anyone predicted it. But economic collapse is different—if enough people believe it will happen, it does. The last straw in the 2008 financial crisis wasn't a data point; it was the contagion of fear itself.

So the strangest thing about this article: if it's right, every decision you make after reading it—whether to save more cash, delay buying a house, switch careers, have a child—makes it more right. You cannot stand outside it. Reading is participation. Understanding is acceleration.

This may be the deepest tragedy of the commons in this entire analysis: not that no one sees the cliff, but that those who see it vote with their feet in a direction that is, precisely, toward the cliff.

But Xiao Ming's story also hides a thread of hope: he paints not for money, runs not to outpace cars, connects with people not for efficiency. **When AI takes everything "useful," what remains is everything "meaningful."** The next chapter of human civilization may not be about competing with AI, but about finding, in a world that no longer requires competition, reasons worth living for.

---

## About This Article

Written by Tony Seah, based on logical extrapolation from public data and research reports. All charts are original hand-drawn SVGs.

## Discussion

Join the Telegram group to discuss the ideas in this article:

![Telegram Group](charts/gwm.jpg)

---

If you found this article valuable, consider supporting the author:

**International:**
- GitHub Sponsors: [github.com/sponsors/pkusnail](https://github.com/sponsors/pkusnail)
- Buy Me a Coffee: [buymeacoffee.com/pkusnail](https://buymeacoffee.com/pkusnail)

**Reprint notice:** Feel free to repost and share this article, but please retain author attribution and support links.

Source / Project: [github.com/pkusnail/GPUs-Wombs-and-Money-Printers](https://github.com/pkusnail/GPUs-Wombs-and-Money-Printers)

---

## References

[92] Cengage Group, "2025 Graduate Employability Report," 2025; The EDU Ledger, "Graduate Job Market Hits Five-Year Low as Skills Gap Widens," 2025; HR Dive, "Over half of hiring managers say recent grads are unprepared for the workforce," 2025. https://www.cengagegroup.com/news/press-releases/2025/cengage-group-2025-employability-report/

[93] Pew Research Center, "Is College Worth It?" May 2024; CNBC, "The median return on investment for a college degree," April 2025; Georgetown CEW, "ROI of College 2025." https://www.pewresearch.org/social-trends/2024/05/23/is-college-worth-it-2/

[94] TechCrunch, "The great computer science exodus (and where students are going instead)," February 2026; CRA CERP Pulse Survey, "A Snapshot of 2025 Undergraduate Computing Enrollment Patterns," October 2025. https://techcrunch.com/2026/02/15/the-great-computer-science-exodus-and-where-students-are-going-instead/

[95] BestColleges, "Colleges That Have Closed: A Running List," 2025; Bryan Alexander, "Academic closures, mergers, and cuts heading into fall 2025"; Federal Reserve Bank of Philadelphia, higher education financial stability projections. https://www.bestcolleges.com/research/closed-colleges-list-statistics-major-closures/

[96] RBA, "International Students and the Australian Economy," July 2025; NAFSA, "International Students Contributed $43 Billion to US Economy 2024-2025"; ICEF Monitor, "UK international student numbers fall for second year," January 2026; ApplyBoard, "Impacts of Canada's 2024 International Student Cap," 2025. https://www.rba.gov.au/publications/bulletin/2025/jul/international-students-and-the-australian-economy.html

[97] Richmond Fed, "Workforce Pell Is (Finally) Law. Now What?" 2025; HCM Strategists, "New Insights for 2025: How States Are Funding and Expanding Short-Term Credential Pathways"; RAND, "Should the U.S. Lean into Short-Term, Nondegree Workforce Education and Training?" June 2025. https://www.richmondfed.org/region_communities/regional_data_analysis/community_college_survey/community_college_insights/2025/workforce_pell_finally_law_now_what

[98] Coursera, "2025 Micro-Credentials Impact Report"; Fortune, "Microcredentials are the hottest hiring trend for 2026," December 2025. https://www.coursera.org/enterprise/resources/ebooks/micro-credentials-report-2025

[99] UK Government, "Lifelong Learning Entitlement: Overview," 2025. https://www.gov.uk/government/publications/lifelong-learning-entitlement-lle-overview/lifelong-learning-entitlement-overview

[100] Worlddidac, "China's Vocational Skills Upgrade 2025-2027: What It Means for Education, Industry, and Global Cooperation," 2025. https://worlddidac.org/news/chinas-vocational-skills-upgrade-2025-2027-what-it-means-for-education-industry-and-global-cooperation/

[101] Stanford Report, "How is ChatGPT impacting schools, really?" July 2025; NBC News, "AI has become the norm for students. Teachers are playing catch-up," 2025. https://news.stanford.edu/stories/2025/07/chatgpt-open-ai-impact-schools-education-learning-data-research

[102] EdTech Hub, "10 Scenarios for Education in 2035," 2025; WEF, "The future of learning: AI is revolutionizing education 4.0," April 2024. https://edtechhub.org/evidence/10-scenarios-for-education-in-2035/

[103] Luo, F. et al., "Impact of Business Cycles on US Suicide Rates, 1928-2007," American Journal of Public Health, 2011; Tapia Granados, J.A., "Recessions and Mortality in the United States, 1920-1940," PNAS, 2009. https://pmc.ncbi.nlm.nih.gov/articles/PMC3134519/

[104] Japanese Cabinet Office, Hikikomori surveys 2019/2023; Tamura et al., "Hikikomori prevalence in Japan," Psychiatry and Clinical Neurosciences, 2025; Nippon.com, "Japan's Hikikomori Population Tops 1.46 Million." https://www.nippon.com/en/japan-data/h01341/

[105] Character.AI usage data via Business of Apps & DemandSage, 2025; Replika via ElectroIQ, 2025; Grand View Research, "AI Companion Market Size Report," 2024 (China: $2.04B revenue, CAGR 35.4%). https://www.businessofapps.com/data/character-ai-statistics/

[106] Chetty, R. et al., "The Association Between Income and Life Expectancy in the United States, 2001-2014," JAMA, 2016; The Lancet, "Life expectancy inequalities in the US, 2000-2019," 2024. https://jamanetwork.com/journals/jama/fullarticle/2513561

[107] Global Times, "AI resurrection of deceased relatives sparks debate in China," March 2024; Euronews, "Digital afterlife: StoryFile, HereAfter AI," 2024; Marina Smith StoryFile funeral case, 2022. https://www.globaltimes.cn/page/202403/1309456.shtml

[108] Nielsen, "The Gauge: Streaming surpasses 44.8% of US TV viewing," May 2025; NPR, "The Monoculture Myth," 2024. https://www.nielsen.com/insights/2025/the-gauge/

[109] Hubert et al., "GPT-4 outperforms humans on divergent thinking," Scientific Reports / Nature, 2023; Nave & Terwiesch, "AI and Idea Diversity," Wharton, 2024; "Creative scar effect," Technological Forecasting and Social Change, 2025. https://www.nature.com/articles/s41598-023-40858-3

[110] RAND, "Dissecting America's AI Action Plan: A Primer for Biosecurity," 2025 (o3 model 94th percentile virology); Anthropic, "Biorisk Evaluations," 2025 (Claude exceeded expert baseline, ASL-3 triggered); RAND Europe, "Global Risk Index for AI-enabled Biological Tools," 2024; CIGI, "AI Is Reviving Fears Around Bioterrorism," 2025 (Paris Summit: 80% improvement in lethal substance instructions). https://www.rand.org/pubs/commentary/2025/08/dissecting-americas-ai-action-plan-a-primer-for-biosecurity.html

[111] NK News, "North Korea can use gene-editing tech to craft military bioweapons, US warns," April 2024; China Ministry of State Security warning on ethnic genetic weapons, October 2023; MIT Kevin Esvelt gene-drive risk estimates; DARPA Safe Genes Program ($65M); AI & Society (Springer), "Synthetic biology/AI convergence security threats," 2025. https://link.springer.com/article/10.1007/s00146-025-02576-4

[112] Small Wars Journal (Arizona State), "AI Use in Terrorist Plots Surges in 2025," December 2025; RAND, "The WMD AI Security Gap," February 2026; War on the Rocks, "AI and the New Blueprint of Terrorism," March 2026; MEMRI, "AI and the New Era of Terrorism," 2025. https://smallwarsjournal.com/2025/12/24/ai-use-in-terrorist-plots-and-attacks-surges-in-2025/

[113] IGI, "CRISPR Clinical Trials 2025 Update" (~250 trials, 150+ active); Casgevy: $2.2M/treatment, 39 patients infused Sept 2025; Cleveland Clinic first-in-human CRISPR cholesterol trial, 2025; Neuralink: 12 implant recipients Sept 2025, Blindsight trial 2026; He Jiankui: relocated Hainan July 2024, new CRISPR embryo proposal funded; Genetic Literacy Project, Global Gene Editing Regulation Tracker (75/96 countries prohibit). https://innovativegenomics.org/news/crispr-clinical-trials-2025/

[114] Rueda, "Genetic enhancement, human extinction, and the best interests of posthumanity," Bioethics (Wiley), 2024; Kurzweil, The Singularity Is Nearer, 2024 (AGI 2029, millionfold intelligence by 2045); Harari, Homo Deus, 2016 (biological caste system). https://onlinelibrary.wiley.com/doi/10.1111/bioe.13085
